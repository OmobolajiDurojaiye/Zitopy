...
document.querySelectorAll('a[href^="#"]').forEach((anchor) => {
  anchor.addEventListener("click", function (e) {
    ...
    if (targetId && targetId.length > 1 && targetId !== "#authModal") {
      // Exclude authModal anchor
...
// window.addEventListener("scroll", () => { // This will be part of debounced scroll
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initialize Auth Modal if params are present (e.g. after form submission redirect)
  handleAuthModalDeepLink();
});
...
const animateCounter = (element, target, duration = 2000) => {
  ...
  if (isNaN(actualTarget)) return; // Guard against NaN
...
buttons.forEach((button) => {
  button.addEventListener("click", (e) => {
    // Prevent ripple on tab/form toggle buttons inside modals
    if (
      button.classList.contains("tab-button") ||
      button.classList.contains("form-toggle-button")
    ) {
      return;
    }
...
if (document.body)
  document.body.appendChild(scrollToTopBtn); // Check if body exists
else
...
const openAuthModalButtons = document.querySelectorAll(".open-auth-modal");
const closeAuthModalBtn = document.getElementById("closeAuthModal");

const mainTabButtons = authModal
  ? authModal.querySelectorAll(".auth-modal-header .tab-button")
  : [];
const mainTabContents = authModal
  ? authModal.querySelectorAll(".tab-content")
  : [];

function openAuthModal(defaultTab = "businessOwnerTab", defaultForm = null) {
  if (authModal) {
    authModal.classList.add("active");
    document.body.style.overflow = "hidden";
    // Set default tab
    switchTab(defaultTab, mainTabButtons, mainTabContents);

    // Set default form within that tab
    ...
    if (activeTabContent) {
      ...
      let formToActivate = defaultForm;
      if (!formToActivate) {
        // If no specific form, default to the first one (usually login)
        formToActivate =
          formToggleButtons.length > 0
            ? formToggleButtons[0].dataset.form
            : null;
      }
      if (formToActivate) {
        switchForm(formToActivate, formToggleButtons, forms);
      }
    }
  }
}
...
function switchForm(formId, formToggleButtons, forms) {
  ...
  // The form ID in HTML is like 'boLoginForm', dataset.form is 'boLogin'
  // We need to match dataset.form for button, and find form by its actual ID
  const formElementId =
    formId +
    (formId.toLowerCase().includes("login") ||
    formId.toLowerCase().includes("signup")
      ? "Form"
      : ""); // Heuristic for ID

  if (buttonToActivate) buttonToActivate.classList.add("active");

  // Try to find the form by a constructed ID if it's a partial match
  // e.g. if formId is 'boLogin', look for 'boLoginForm'
  let formToActivate = document.getElementById(formId); // Try direct ID first
  if (!formToActivate) {
    // If not found, try constructing
    const potentialFormId = formId.endsWith("Form") ? formId : formId + "Form";
  ...
  if (formToActivate) {
    ...
    // Fallback: if formId was 'boLogin', activate 'boLoginForm'
...
if (authModal) {
  openAuthModalButtons.forEach((button) => {
    button.addEventListener("click", (e) => {
      e.preventDefault();
      openAuthModal(); // Open with default tab (Business Owner)
    });
  });
  ...
  mainTabButtons.forEach((button) => {
    button.addEventListener("click", () => {
      switchTab(button.dataset.tab, mainTabButtons, mainTabContents);
      // Reset to login form when switching main tabs
      ...
      if (activeTabContent) {
        ...
        if (formToggleButtons.length > 0) {
          // Default to the first form (login)
          switchForm(
            formToggleButtons[0].dataset.form,
            formToggleButtons,
            forms
  ...
  // Event delegation for form toggle buttons within each tab content
...
// Handle deep linking to auth modal (e.g., after form submission redirect)
function handleAuthModalDeepLink() {
  const params = new URLSearchParams(window.location.search);
  const anchor = window.location.hash.substring(1); // Get anchor without #
  const modalTab = params.get("modal_tab");
  const modalForm = params.get("modal_form");

  if (anchor === "authModal" && (modalTab || modalForm)) {
    openAuthModal(modalTab || "businessOwnerTab", modalForm);
    // Clean up URL params after opening modal to prevent re-trigger on refresh
    if (history.pushState) {
      const newUrl =
        window.location.pathname + window.location.hash.split("?")[0]; // Keep anchor, remove query params
      history.pushState({ path: newUrl }, "", newUrl);
    }
  }
}
// Call on load if there are params from redirect
// DOMContentLoaded listener already calls this

// Form handling for general CTA/Sign-up buttons (placeholder functionality)
const signupDemoButtons = document.querySelectorAll(
  ".hero-buttons .btn-primary-large, .hero-buttons .btn-secondary-large, .cta-buttons .btn-primary-large, .cta-buttons .btn-secondary-large"
);

signupDemoButtons.forEach((button) => {
  button.addEventListener("click", (e) => {
    if (
      button.id === "watchDemoBtn" ||
      button.classList.contains("open-auth-modal")
    ) {
      // These are handled by their specific listeners
      return;
    }
    // ... (rest of your generic button logic, if any) ...
    const buttonTextContent = button.textContent || button.innerText;
    const buttonText = buttonTextContent.toLowerCase().trim();
    if (
      buttonText.includes("schedule a demo") ||
      buttonText.includes("contact")
    ) {
      console.log("Action: Schedule a demo or contact (generic handler).");
    }
  });
});
...
document.addEventListener("DOMContentLoaded", () => {
  console.log("ZITOPY Landing Page (with Auth Modal) loaded successfully!");
